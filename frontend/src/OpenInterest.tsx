...
export default function OpenInterest() {
  ...
  useEffect(() => {
    ...
    const start = setTimeout(async () => {
      try {
        const response = await fetch('/api/scanner/open-interest?symbol=BTCUSDT', { signal: controller.signal, cache: 'no-store' })
        if (!response.ok) throw new Error(`OI Bybit недоступен (HTTP ${response.status}).`)
        const result: Result = await response.json()
        if (result.exchange !== 'bybit' || result.symbol !== 'BTCUSDT' || result.unit !== 'BTC'
  ...
  return <div className="momentum-panel">
    <h3>Open Interest · Bybit · BTCUSDT</h3>
    ...
    {data && <>
      <p className="price-reference">Последнее измерение: <strong>{Number(data.open_interest).toLocaleString('ru-RU', { maximumFractionDigits: 8 })} BTC</strong> · {new Date(data.measured_at).toLocaleString('ru-RU')} (местное время)</p>
